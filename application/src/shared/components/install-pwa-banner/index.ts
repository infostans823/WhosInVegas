...
export class InstallPwaBannerComponent implements OnInit {
    ...
    private defaultShortPeriod: number = 60; // 1 minute
    private defaultLongPeriod: number = 3600 * 24 * 2;
    ...
    install(): void {
        ...
        this.setInstallPwaBannerExpireTime(this.defaultLongPeriod);
        this.ref.markForCheck();
    }
    ...
    close(): void {
        const confirm = this.alert.create({
            ...
            message:'Sure you don’t want to add Who’s in Vegas to your home screen?',
            buttons: [{
                text: this.translate.instant('pwa_install_close_confirm_dismiss'),
                handler: () => {
                    this.setInstallPwaBannerExpireTime(this.defaultShortPeriod);
                    this.ref.markForCheck();
                }
            }, {
